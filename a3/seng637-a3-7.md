...
### Test Case 2 Range Contains Method 
The Range Contains method has two main check that be seen in the code snippet below. The method is called on a range object and passes in double. The first check ensures that double provided is not below the lower value of the range object. The next check ensures that the provided value is not above the upper value of the range object. If both of these checks pass the method returns a string informing that the value is within then range. 
...
```
The following three tests were written in order to test the two checks where the value was either above or below the range values. The next test was written to unsure that the confirmation message was brought back showing that the passed in value was contained inside the range. 
